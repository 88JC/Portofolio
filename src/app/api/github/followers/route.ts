...
export async function GET(request: Request) {
  try {
    ...
    while (hasMore) {
      const response = await fetch(`https://api.github.com/users/${username}/followers?per_page=100&page=${page}`, { headers });
      if (response.ok) {
    ...
    return NextResponse.json({ followers: allFollowers });
